...
if (!empty($_POST)) {
    if (empty($_POST['nombre']) || empty($_POST['apellido_paterno']) || empty($_POST['apellido_materno']) || empty($_POST['nombre_usuario']) || empty($_POST['tipo_usuario']) || empty($_POST['id_rol'])) {
        $respuesta = array(
            'status' => false, 
            'msg' => 'Todos los campos requeridos son necesarios - ajax',
        );
    } else {
        ...
        if ($result) {
            ...
            if (empty($idpadre)) {
                ...
                $request = $queryInsert->execute(array($nombre, $apellido_paterno, $apellido_materno, $nombre_usuario, $contraseña, $tipo_usuario, $numero_contacto, $id_rol, $info_contacto, $est_reg));
                $accion = 1;
            } else {
                if (empty($contraseña)) {
                    ...
                    $request = $queryUpdate->execute(array($nombre, $apellido_paterno, $apellido_materno, $nombre_usuario, $tipo_usuario, $numero_contacto, $id_rol, $info_contacto, $est_reg, $idpadre));
                    $accion = 2;
                } else {
                    ...
                    $queryUpdate = $pdo->prepare($sqlUpdate);
                    $request = $queryUpdate->execute(array($nombre, $apellido_paterno, $apellido_materno, $nombre_usuario, $contraseña, $tipo_usuario, $numero_contacto, $id_rol, $info_contacto, $est_reg, $idpadre));
                    $accion = 3;
                }
            }
